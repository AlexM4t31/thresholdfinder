...
class ImageReviewerApp:
    def __init__(self, root):
        ...
        tk.Label(root, text="Combo index:").grid(row=1, column=0, sticky="w", padx=5)
    ...
    def get_current_combid_statelist(self):
        tmp_c_id = self.combo_id_list[self.current_combo_index]

        tmp_crt_combo_id_states = self.combo_id_to_instance_state_list[tmp_c_id]

        return (tmp_c_id,tmp_crt_combo_id_states)


    def find_undecided_instance_index_in_current_state_list(self, st_idx):    
        tmp_c_id, tmp_crt_combo_id_states = self.get_current_combid_statelist()

        inst_n = len(tmp_crt_combo_id_states)

        try: 
            und_idx = next( idx for idx in range(st_idx, inst_n) if tmp_crt_combo_id_states[idx] == -1 )
        except:
            und_idx = inst_n - 1
    ...
    def show_current_index(self):
        tmp_c_id, tmp_crt_combo_id_states = self.get_current_combid_statelist()
        self.current_shown_combo_index = tmp_c_id
    ...
    def load_current_combo_id(self):
        ...
        self.show_image_for_cid_and_instid(tmp_c_id, self.current_instance_index)
    ...
    def show_image_for_cid_and_instid(self):   
    ...
    def detect_crt_combo_id_decision_state(self):
        ...
        for window_starting_idx in range(inst_n - self.SLIDING_WINDOW_SIZE + 1):
            tmp_sublist = tmp_crt_combo_id_states[window_starting_idx : (window_starting_idx + self.SLIDING_WINDOW_SIZE) ]
    ...
    def set_current_instance_state_to_value(self, in_val):
        ...
        tmp_crt_combo_id_states[self.current_instance_index] = in_val 
